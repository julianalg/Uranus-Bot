...
async def on_message(message):
  ...
  if message.content == '?murder mystery':
    guild = bot.guilds[0]
    
    members = guild.members
    
    await message.channel.send(f"**murder mystery:** \n victim: {random.choice(members)} \n found body: {random.choice(members)} \n lead detective: {random.choice(members)}\n first suspect: {random.choice(members)} \n suspect's alibi: {random.choice(members)} \n final 3 suspects: {random.choice(members)}, {random.choice(members)}, {random.choice(members)} \n who did it: {random.choice(members)} \n went to jail: {random.choice(members)}")
